...
-- NVL(칼럼이름, 특정값) : 칼럼이름 NULL인 경우는 특정값을 출력, NULL이 아니면 칼럼값 출력
...
-- NULL을 0으로 바꿈
SELECT COUNT(NVL(CNT, 0)) 개 FROM k1; --5

SELECT SUM(NVL(CNT, 0))/4 합계 FROM k1; --5

SELECT AVG(NVL(CNT, 0)) 평균 FROM K1; -- 4
-- NULL을 5로 변경
SELECT MIN(NVL(CNT, 5)) 최소값 FROM K1;
